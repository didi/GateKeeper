...
func (p *Proxy) rrConfigFor(ipPort string) *config {
	if p.configs == nil {
		p.configsLock = &sync.RWMutex{}
		p.configs = make(map[string]*config)
...
func (p *Proxy) addWeightRoute(ipPort string, weight int, r route) {
	cfg := p.rrConfigFor(ipPort)
	cfg.rr.Add(r, weight)
}
...
//Use proxy use middleware
func (p *Proxy) Use(ipPort string, mws ...HandlerFunc) *Proxy {
	p.configsLock.Lock()
	p.mwsCb[ipPort] = func(descRouter *TCPRouter) *TCPRouter {
		descRouter = descRouter.Use(mws...)
		return descRouter
	}
	p.configsLock.Unlock()
	return p
...
//AddWeightRoute proxy add a weightroute
func (p *Proxy) AddWeightRoute(ipPort string, weight int, dest Target) {
	p.addWeightRoute(ipPort, weight, fixedTarget{dest})
}
...
func (p *Proxy) Start() error {
	...
	for ipPort, config := range p.configs {
		...
		if config.rr != nil {
			go p.serveRRListener(errc, ln, config.rr, ipPort)
		} else {
...
func (p *Proxy) serveRRConn(c net.Conn, rr public.RR, ipPort string) bool {
...
// to determine how to route the connection. The Read method stitches	缝合了
