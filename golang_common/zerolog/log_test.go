...
func TestInfo(t *testing.T) {
	t.Run("empty", func(t *testing.T) {
		...
		log := New(out)
		log.Info().Msg("")
		if got, want := decodeIfBinaryToString(out.Bytes()), `[INFO]`+"\n"; got != want {
			t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
		}
	})
	...
	t.Run("one-field", func(t *testing.T) {
		...
		if got, want := decodeIfBinaryToString(out.Bytes()), `[INFO]||foo=bar`+"\n"; got != want {
			t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
		}
	})
	...
	t.Run("two-field", func(t *testing.T) {
		...
		log.Info().
			...
			Int("n", 123).
			Msg("")
		if got, want := decodeIfBinaryToString(out.Bytes()), `[INFO]||foo=bar||n=123`+"\n"; got != want {
			t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
		}
	})
}
...
func TestFieldsArrayEmpty(t *testing.T) {
	...
	log.Log().
		...
		Msg("")
	if got, want := decodeIfBinaryToString(out.Bytes()), `string=[]||bool=[]||int=[]||int8=[]||int16=[]||int32=[]||int64=[]||uint=[]||uint8=[]||uint16=[]||uint32=[]||uint64=[]||float32=[]||float64=[]||dur=[]||time=[]`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
...
func TestFieldsArraySingleElement(t *testing.T) {
	...
	log.Log().
		...
		Msg("")
	if got, want := decodeIfBinaryToString(out.Bytes()), `string=[foo]||bool=[true]||int=[1]||int8=[2]||int16=[3]||int32=[4]||int64=[5]||uint=[6]||uint8=[7]||uint16=[8]||uint32=[9]||uint64=[10]||float32=[11]||float64=[12]||dur=[1000]||time=["0001-01-01T00:00:00Z"]`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
...
func TestFieldsArrayMultipleElement(t *testing.T) {
	...
	log.Log().
		...
		Msg("")
	if got, want := decodeIfBinaryToString(out.Bytes()), `string=[foo,bar]||bool=[true,false]||int=[1,0]||int8=[2,0]||int16=[3,0]||int32=[4,0]||int64=[5,0]||uint=[6,0]||uint8=[7,0]||uint16=[8,0]||uint32=[9,0]||uint64=[10,0]||float32=[11,0]||float64=[12,0]||dur=[1000,0]||time=["0001-01-01T00:00:00Z","0001-01-01T00:00:00Z"]`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
...
func TestMsgf(t *testing.T) {
	out := &bytes.Buffer{}
	log := New(out)
	log.Debug().Msgf("one %s %.1f %d %v", "two", 3.4, 5, errors.New("six"))
	if got, want := decodeIfBinaryToString(out.Bytes()), `[DEBUG]||one two 3.4 5 six`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
...
func TestLevel(t *testing.T) {
	...
	t.Run("NoLevel/Info", func(t *testing.T) {
		...
		log := New(out).Level(InfoLevel)
		log.Info().Msg("test")
		if got, want := decodeIfBinaryToString(out.Bytes()), `[INFO]||test`+"\n"; got != want {
			t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
		}
	})
...
func TestOutputWithoutTimestamp(t *testing.T) {
	...
	if got, want := decodeIfBinaryToString(out.Bytes()), `foo=bar||hello world`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
